...
// 그래서 필요한 쪽을 PNG 로 렌더링해 미리보기·별지 첨부에 함께 쓴다.
// (여러 건이 든 PDF 는 건마다 그 건이 있는 쪽을 렌더링한다.)
...
export interface PdfPreview {
  pages: Record<number, string>; // 쪽 번호 → PNG 데이터 URL
  pageCount: number;
}

/**
 * PDF 의 지정한 쪽들 → PNG 데이터 URL. 실패하면 예외를 던진다(호출부에서 인식은 계속 진행).
 * 문서를 한 번만 열어 여러 쪽을 렌더링한다. 범위를 벗어난 쪽 번호는 잘라 맞춘다.
 */
export async function renderPdfPages(file: File, pageNos: number[] = [1], targetWidth = 1000): Promise<PdfPreview> {
  const pdfjs = await loadPdfjs();
  const data = new Uint8Array(await file.arrayBuffer());
  const task = pdfjs.getDocument({ data });
  const doc = await task.promise;
  try {
    const wanted = [...new Set(pageNos.map((n) => Math.min(doc.numPages, Math.max(1, Math.round(n) || 1))))];
    ...
    for (const no of wanted) {
      ...
      pages[no] = canvas.toDataURL('image/png');
    }
