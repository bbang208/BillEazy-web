...
const SAMPLES: ReceiptExtraction[] = [
  {
    ...
    confidence: 0.92, matched_keywords: ['건전지', '리모컨', '스토어'], page: 1,
  },
  {
    ...
    confidence: 0.72, matched_keywords: ['항공'], page: 1,
  },
  {
    ...
    confidence: 0.9, matched_keywords: ['스타벅스', '커피'], page: 1,
  },
  {
    ...
    payment_method: 'card', routing_hint: 'personal_expense', account_suggestion: '여비교통비',
    confidence: 0.9, matched_keywords: ['주유', '에너지'], page: 1,
  },
  {
    ...
    confidence: 0.86, matched_keywords: ['우체국', '우편'], page: 1,
  },
  {
    ...
    confidence: 0.45, matched_keywords: [], page: 1,
  },
];
 ...
 * multi=true(PDF)면 3건을 쪽별로 돌려줘서 "한 파일 → 여러 항목" 분리 흐름도 테스트할 수 있다.
 */
export async function mockExtract(multi = false): Promise<ReceiptExtraction[]> {
  await new Promise((r) => setTimeout(r, 500 + Math.random() * 500));
  const count = multi ? 3 : 1;
  const out: ReceiptExtraction[] = [];
  for (let i = 0; i < count; i++) {
    out.push({ ...SAMPLES[idx % SAMPLES.length], page: i + 1 });
    idx += 1;
  }
  return out;
}
